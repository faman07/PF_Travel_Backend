...
excursionRouter.get('/getActivityAPI', async (req, res) => { 
  console.log("Estamos en getActivityAPI")
  const {latitude, longitude, radius, country, city, store} = req.body
    try {
        const activityData =  await getExcursionsApi({latitude, longitude, radius})
        if (activityData && store == "yes") {
          ...
          for (const u of formatData) {
            ...
            console.dir(reformatData)
            //const activities = await storeActivity(reformatData)
          }
        } else {
        return res.send(activityData)
        }
        console.log("Solo se muestran resultados, no se gusrdan en la base de datos")
        return res.send(activityData)
      }  
      catch (e) {
